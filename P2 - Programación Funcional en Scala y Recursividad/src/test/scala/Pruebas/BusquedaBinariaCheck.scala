...
object BusquedaBinariaCheck extends org.scalacheck.Properties("Test Búsqueda Binaria"){
  val MAX_TAM = 50
  val MAX_VAL = 25
  ...
  // Test con enteros
  property("Búsqueda binaria sobre enteros coincide con IndexOf") = {
    forAll(problemaEnteros) {
      problema => {
        ...
        busquedaBinaria(coleccion, valor, criterioInt) == coleccion.indexOf(valor)
      }
    }
  }

  // Test con char
  property("Búsqueda binaria sobre char coincide con IndexOf") = {
    forAll(problemaChar) {
      problema => {
        val coleccion  = problema._1.sorted.toArray
        val valor = problema._2
        println("Comprobación con una lista de char")
        busquedaBinaria(coleccion, valor, criterioChar) == coleccion.indexOf(valor)
      }
    }
  }
}
